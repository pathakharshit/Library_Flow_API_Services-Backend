...
@Component
public class BooksFilterByNumberOfCopies implements FilterBooks {
    @Autowired
    ...
    public List<Book> filterAllBooks(FilterOperator operator, String value) {
        ...
        for(Book book : books) {
            if(book.getNumberOfCopies() == val) booksEqualsToValue.add(book);
            else if(book.getNumberOfCopies() < val) booksLessThanValue.add(book);
